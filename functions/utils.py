...
#%% PyTorch Class for Optimization Problems

class TestProblem(torch.nn.Module):
    """
    Converts optimization problem to torch neural network module.
    
    Implemented by: Hao-Jun Michael Shi and Dheevatsa Mudigere
    Last edited 9/4/18.
    
    Inputs:
        problem (callable): optimization problem with form x = problem(x, dim, init)
        dim (int): compatible dimension
    ...
    def __init__(self, problem, dim):
        ...
        # get initialization
        x = problem(0, dim, True)[0]
        x.requires_grad_()
        
        # store variables, problem, and dimension
        self.variables = torch.nn.Parameter(x)
        self.problem = problem
        self.dim = dim
    
    def forward(self):
        return self.problem(self.variables, self.dim, False)
