"""
Multi-Batch L-BFGS Implementation

Demonstrates how to implement a simple multi-batch L-BFGS with a probabilistic 
Armijo backtracking line search to train a simple convolutional neural network 
using the LBFGS optimizer. Multi-batch L-BFGS is a stochastic quasi-Newton method 
that performs curvature pair updates over the overlap between consecutive samples
...
for n_iter in range(max_iter):
    ...
    obj = overlap_ratio*(obj_Ok_prev + obj_Ok) + (1 - 2*overlap_ratio)*obj_Nk
    ...
    options = {'closure': closure, 'current_loss': obj, 'eta': 2, 'interpolate': True}
    obj_Ok_prev, lr, _, _, _ = optimizer.step(p, g_Ok, options)
    
    # compute overlap gradient at new iterate
    Ok_prev = Ok
    obj_Ok_prev.backward()
    grad_Ok_prev = optimizer._gather_flat_grad()
